...
class Block {
  ...
  private:
    ...
    const DataRecord **records; // List of this->records
    uint32_t recordsNumber; // Number of this->records
  ...
  public:
    ...
    const DataRecord *get(const int index); // Return ptr to idx record
    uint32_t count();                 // Return number of this->records

    void write(const DataRecord *r, const int pos=-1);     // Write Record r in this
